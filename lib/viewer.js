...
export default class Viewer {
  ...
  sendMessage(data) {
    try {
      ...
      atom.notifications.addError(`pdf-viewer: Cannot send message to PDFjs: ${err}`)
    }
  }
  ...
  handleSynctex(data) {
    ...
    cp.exec(cmd, (err, stdout, stderr) => {
      ...
      if (!synctex.input) {
        ...
        return atom.notifications.addError(`pfd-viewer: cannot open synctex input "${synctex.input}", because it does not exists`)
      } else {
