...
module.exports = class Viewer {
  ...
  getVendor() {
    return `pdfjs-${pkg.pdfjsVersion}-dist`;
  }
  ...
  handleKeydown(data) {
    ...
    } else if (data.action === "fuzzy-files:toggle") {
      atom.commands.dispatch(
        atom.views.getView(atom.workspace),
        "fuzzy-files:toggle"
      );
    } else if (data.action === "fuzzy-explorer:toggle") {
      atom.commands.dispatch(
        atom.views.getView(atom.workspace),
  ...
  async handleSynctex(data) {
    ...
    const result = await latexTools.syncToSource(
      this.filePath,
      data.pageNo,
      data.x,
      data.y
  ...
  /**
   * Triggers LaTeX compilation for this PDF's source file.
   */
  compile() {
    ...
    if (!fs.existsSync(texFile)) {
      if (this.debug) {
        console.log(`[pdf-viewer] No .tex file found for ${path.basename(this.filePath)}`);
      }
      atom.notifications.addWarning(
        `pdf-viewer: No .tex file found for ${path.basename(this.filePath)}`
      );
      return;
    }
    ...
    if (!latexTools?.compile) {
      ...
      atom.notifications.addWarning(
        "pdf-viewer: latex-tools not available"
      );
      return;
    }
  ...
  /**
   * Opens the corresponding .tex source file.
   */
  openTex() {
    const texFile = this.filePath.replace(/\.pdf$/, ".tex");
  ...
  messageEvent(message) {
    if (message.source !== this.element.contentWindow) {
      ...
      this.emitScrollmapData(message.data);
