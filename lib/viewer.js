...
module.exports = class Viewer {
  ...
  pauseAutoRefresh() {
    ...
    if (this.autoRefresh && !this.autoRefreshPausedByBuild) {
      if (this.debug) {
        console.log(`[pdf-viewer] Pausing auto-refresh for ${path.basename(this.filePath)} during build`);
      }
  ...
  resumeAutoRefresh() {
    ...
    if (this.autoRefreshPausedByBuild) {
      if (this.debug) {
        console.log(`[pdf-viewer] Resuming auto-refresh for ${path.basename(this.filePath)} after build`);
      }
      this.autoRefreshPausedByBuild = false;
      ...
      if (this.autoRefresh) {
        if (this.ready) {
          ...
          if (this.debug) {
            console.log(`[pdf-viewer] Viewer not ready, scheduling pending refresh for ${path.basename(this.filePath)}`);
          }
  ...
  checkFileStability() {
    try {
      ...
      if (currentSize === 0) {
        ...
        if (this.debug) {
          console.log(`[pdf-viewer] File is empty, waiting for content: ${path.basename(this.filePath)}`)
        }
      ...
      if (currentSize !== this.lastFileSize) {
        ...
        if (this.debug) {
          console.log(`[pdf-viewer] File size changed (${this.lastFileSize} -> ${currentSize}), waiting: ${path.basename(this.filePath)}`)
        }
      ...
      if (this.isPdfValid()) {
        if (this.debug) {
          console.log(`[pdf-viewer] File stable and valid, refreshing: ${path.basename(this.filePath)}`)
        }
        this.refresh()
      } else {
  ...
  handleSynctex(data) {
    ...
    cp.exec(cmd, (err, stdout, stderr) => {
      if (err && this.debug) { return console.error(`pdf-viewer: synctex error: ${stderr}`) }
      let synctex = {}
  ...
  messageEvent(message) {
    if (message.source !== this.element.contentWindow) {
      ...
      if (this.pendingRefresh) {
        if (this.debug) {
          console.log(`[pdf-viewer] Viewer now ready, executing pending refresh for ${path.basename(this.filePath)}`);
        }
        this.pendingRefresh = false;
        this.refresh();
      }
