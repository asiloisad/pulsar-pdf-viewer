...
module.exports = {
  ...
  compileFromViewer() {
    ...
    if (!fs.existsSync(texFile)) {
      if (atom.config.get('pdf-viewer.debug')) {
        console.log(`[pdf-viewer] No .tex file found for ${path.basename(pdfFile)}`);
      }
      atom.notifications.addWarning(`pdf-viewer: No .tex file found for ${path.basename(pdfFile)}`);
      return;
    }
    ...
    if (!this.buildService) {
      if (atom.config.get('pdf-viewer.debug')) {
        console.log('[pdf-viewer] Build service not available (latex-tools not active?)');
      }
      atom.notifications.addWarning('pdf-viewer: latex-tools build service not available');
      return;
    }
    ...
    // Trigger compilation
    if (atom.config.get('pdf-viewer.debug')) {
      console.log(`[pdf-viewer] Triggering compilation of ${path.basename(texFile)}`);
    }
  ...
  synctex() {
    ...
    if (!file || !file.endsWith(".tex")) {
      if (atom.config.get('pdf-viewer.debug')) {
        console.error(`pdf-viewer: Cannot run synctex, because source "${file}" is not TeX file`)
      }
      return
    }
    ...
    if (!fs.existsSync(pdfFile)) {
      if (atom.config.get('pdf-viewer.debug')) {
        console.error(`pdf-viewer: Cannot run synctex, because "${pdfFile}" does not exists`)
      }
      return
    }
