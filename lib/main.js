...
export default {
  ...
  synctex() {
    ...
    if (!file || !file.endsWith(".tex")) {
      return console.log(`pdf-viewer: Cannot run synctex, because source "${file}" is not TeX file`)
    }
    pdfFile = file.substr(0, file.lastIndexOf(".")) + ".pdf";
    if (!fs.existsSync(pdfFile)) {
      return console.log(`pdf-viewer: Cannot run synctex, because "${pdfFile}" does not exists`)
    }
