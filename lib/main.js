...
module.exports = {
  ...
  activate() {
    ...
    this.disposables.add(
      ...
      atom.config.observe("pdf-viewer.scrollMapState", () => this.updateScrollMapState())
    );
  },
  ...
  createViewer(filePath, hash) {
    ...
    // Enable scroll-map if service is available and enabled
    if (this.SimpleMap && atom.config.get("pdf-viewer.scrollMapState")) {
      viewer.enableScrollMap(this.SimpleMap);
    }
    return viewer;
  },
  ...
  /**
   * Consumes the simplemap service for outline markers.
   * @param {Function} SimpleMap - The SimpleMap class
   * @returns {Disposable} Disposable to unregister the service
   */
  consumeSimpleMap(SimpleMap) {
    if (atom.config.get("pdf-viewer.debug")) {
      console.log("[pdf-viewer] Consuming simplemap service");
    }
    this.SimpleMap = SimpleMap;
    this.updateScrollMapState();
    return new Disposable(() => {
      if (atom.config.get("pdf-viewer.debug")) {
        console.log("[pdf-viewer] Disposing simplemap service");
      }
      // Disable scroll-map on all viewers
      for (let viewer of this.viewers) {
        viewer.disableScrollMap();
      }
      this.SimpleMap = null;
    });
  },
  ...
  /**
   * Updates scroll-map state on all viewers based on config.
   */
  updateScrollMapState() {
    if (!this.SimpleMap) {
      return;
    }
    const enabled = atom.config.get("pdf-viewer.scrollMapState");
    for (let viewer of this.viewers) {
      if (enabled) {
        viewer.enableScrollMap(this.SimpleMap);
      } else {
  ...
  synctex() {
    ...
    if (!fs.existsSync(pdfFile)) {
      if (atom.config.get("pdf-viewer.debug")) {
        console.error(
          `pdf-viewer: Cannot run synctex, because "${pdfFile}" does not exist`
        );
      }
      return;
    }
    ...
    cp.exec(command, (err, stdout, stderr) => {
      ...
      if (!synctex.page) {
        if (atom.config.get("pdf-viewer.debug")) {
          console.error(`pdf-viewer: synctex stdout parsed with errors`);
        }
        return;
      }
      for (let viewer of this.viewers) {
