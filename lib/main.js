...
module.exports = {
  ...
  createViewer(filePath, hash) {
    ...
    if (this.scrollMapService && atom.config.get("pdf-viewer.scrollMapState")) {
      viewer.enableScrollMap(this.scrollMapService);
    }
    return viewer;
  },
  ...
  /**
   * Consumes the scroll-map service for outline markers.
   * @param {Object} service - The scroll-map service
   * @returns {Disposable} Disposable to unregister the service
   */
  consumeScrollMap(service) {
    if (atom.config.get("pdf-viewer.debug")) {
      console.log("[pdf-viewer] Consuming scroll-map service");
    }
    this.scrollMapService = service;
    this.updateScrollMapState();
    return new Disposable(() => {
      if (atom.config.get("pdf-viewer.debug")) {
        console.log("[pdf-viewer] Disposing scroll-map service");
      }
